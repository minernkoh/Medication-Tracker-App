...
function CalendarDate({
  ...
  const getStyles = () => {
    ...
    // Default state - check if today (just stroke, no background)
    if (isToday) {
      return {
        backgroundColor: colors.background.default,
  ...
  return (
    <div
      className={`flex flex-col h-[3.75rem] items-center justify-center ${pxValue} ${paddingY} rounded-lg border border-solid flex-1 min-w-[3.75rem] cursor-pointer transition-all duration-200 relative`}
      style={{
    ...
    >
      <p
        className="font-poppins leading-6 text-sm shrink-0 font-normal"
