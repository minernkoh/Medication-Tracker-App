...
function PieChart({ taken = 0, notTaken = 0, size = 120 }) {
  ...
  const getTakenPath = () => {
    ...
    if (takenPercentage === 100) {
      ...
      const outerPath = `M ${centerX} ${centerY - outerRadius} A ${outerRadius} ${outerRadius} 0 1 1 ${centerX} ${centerY + outerRadius} A ${outerRadius} ${outerRadius} 0 1 1 ${centerX} ${centerY - outerRadius}`;
      const innerPath = `M ${centerX} ${centerY - innerRadius} A ${innerRadius} ${innerRadius} 0 1 0 ${centerX} ${centerY + innerRadius} A ${innerRadius} ${innerRadius} 0 1 0 ${centerX} ${centerY - innerRadius}`;
    ...
    <div className="flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        {taken > 0 && (
          <path
            d={getTakenPath()}
            fill="#10b981" // Green color for success
